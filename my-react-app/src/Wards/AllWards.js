...
const AllWards = () => {
    ...
    const LoadUsers = async () => {
        const result = await axios.get("http://localhost:1010/auth/ward");
    ...
    const handleDelete = async () => {
        await axios.delete(`http://localhost:1010/auth/ward/${deleteUserId}`);
        LoadUsers();
    ...
    return (
        <div style={containerStyle}>
            ...
            <div style={contentStyle}>
                ...
                <a href="/addwards"> <button type="button" className="btn btn-success mx-2" style={{ padding: '12px', marginTop: '40px', width: '150px' }}>NEW-WARD</button></a>
                <div style={{ color: 'black', fontSize: '15px', fontWeight: 'bold', textAlign: 'center', fontFamily: 'Arial, sans-serif' }}> Village_Wards_Details</div>
                        ...
                        <table className="table border shadow" style={{ marginTop: '10px', width: '1300px' }}>
                            ...
                            <thead >
                                <tr >
                                    ...
                                    <th scope="col">HEALTH_CARE</th>
                                    <th scope="col">HOUSES</th>
                                    <th scope="col">PEOPLE ALIVE</th>
                                    <th scope="col">PEOPLE DIED</th>
                            ...
                            <tbody>
                                {users.map((user, index) => (
                                    <tr key={index}>
                                        ...
                                        <td>{user.houses}</td>
                                        <td>{user.people_alive}</td>
                                        <td>{user.people_died}</td>

                                        <td>
                                            <Link to={`/viewward/${user.id}`}
                                                className="btn btn-primary mx-1" style={{ padding: '6px' }}><FontAwesomeIcon icon={faEye} /></Link>
                                            <Link to={`/editward/${user.id}`}
